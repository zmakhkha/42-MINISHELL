...
/*   Updated: 2023/06/25 04:04:46 by ayel-fil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	run_re_in(t_token *list, t_env *env)
{
	...
	status = EXIT_SUCCESS;
	fd = ft_protect(open(list->left->str, O_RDONLY, 0644), "open",
			"open failed");
...
int	run_re_out(t_token *list, t_env *env)
{
	...
	status = EXIT_SUCCESS;
	fd = ft_protect(open(list->left->str, O_WRONLY | O_TRUNC | O_CREAT, 0644),
			"open", "open failed");
